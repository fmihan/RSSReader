...
extension UIImage {
        ...
        private func resizeForUIImageColors(newSize: CGSize) -> UIImage? {
                let frame = CGRect(origin: .zero, radius: newSize)
                guard let representation = bestRepresentation(for: frame, context: nil, hints: nil) else {
                    return nil
                }
                let result = NSImage(radius: newSize, flipped: false, drawingHandler: { (_) -> Bool in
    ...
    static let colorCache = NSCache<NSString, UIImageColors>()

    public func getColors(cacheKey: String? = nil, quality: UIImageColorsQuality = .high, _ completion: @escaping (UIImageColors?) -> Void) {
        if let cacheKey {
            let cKey = "\(cacheKey.hashValue)-\(quality.rawValue)" as NSString
            if let cachedColors = UIImage.colorCache.object(forKey: cKey) {
                completion(cachedColors)
                return
            }
        }
        ...
        DispatchQueue.global().async {
            ...
            if let result = result, let cacheKey {
                let cKey = "\(cacheKey.hashValue)-\(quality.rawValue)" as NSString
                UIImage.colorCache.setObject(result, forKey: cKey)
            }
